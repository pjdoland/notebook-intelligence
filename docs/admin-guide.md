...
| `NBI_CLAUDE_TOOL_RESULT_MAX_OUTPUT_TOKENS`       | int  | `10000`                     | env                                | Approximate per-result token cap for Claude's cell-output and terminal tools. A numeric zero restores unbounded output; other values are clamped to `256`–`100000`, so negative values cannot disable the cap. Oversized results retain their beginning and end with a truncation marker; unexpected structured returns are stringified before applying the same bound. Resolved at server startup; restart JupyterLab after changing it.                                                                                                                                                                                                                                                                                                         |
...
### Gating the experimental ACP agent (#378)

ACP mode runs an external coding agent over the Agent Client Protocol; Codex (via the codex-acp adapter) is the first selectable agent type. ACP mode and Claude mode are mutually exclusive: enabling one disables the other on save, and a hand-edited config that enables both resolves to Claude. The admin controls that apply:

- **`NBI_ACP_MODE_POLICY`** (default `force-off`): whether ACP mode can be enabled at all. While `force-off`, the ACP settings tab is hidden and `enabled` is clamped to false on every config write, so it is the nuclear off switch.
- **`NBI_ACP_FULL_ACCESS_POLICY`** (default `force-off`): whether the agent may run tools without asking. While full access is off, NBI pins Codex to `approval_policy = untrusted`, so it asks before anything beyond trusted read-only commands and each request surfaces through NBI's per-tool confirmation. With full access on, NBI pins `approval_policy = never` and `sandbox_mode = workspace-write`, so Codex edits files and runs commands without asking, inside Codex's workspace-write sandbox. That sandbox limits writes to the workspace (the Jupyter server's root directory) and temp directories and keeps outbound network access off, unless Codex's own config widens it. Because Codex never asks under full access, anything the sandbox blocks, such as a `pip install` that needs the network or a write outside the workspace, fails instead of prompting. `user-choice` exposes the "Full access" toggle in the ACP tab; `force-on` requires unattended runs. Like bypass, the value is clamped server-side, so a hand-crafted config write can't relax it.
- **Credentials and model** follow the same env-override locks as Claude: `OPENAI_API_KEY`, `OPENAI_BASE_URL`, and `NBI_ACP_CHAT_MODEL` pin and hide the corresponding fields.

How the pins are applied: NBI passes `approval_policy` (and, with full access, `sandbox_mode`) to codex via the codex-acp binary's `-c key=value` command-line override, which takes precedence over Codex's user and project config (`$CODEX_HOME/config.toml` and a workspace's `.codex/config.toml`). System-level and managed Codex configuration under `/etc/codex` is still read, and managed configuration outranks NBI's pins. When an API key is configured, NBI also runs codex with an isolated `CODEX_HOME` under the NBI user directory, so neither the workspace's nor the user's `~/.codex` config is read (this isolates configuration only; the agent still operates on workspace files, subject to the approval and sandbox pins). With ChatGPT (OAuth) auth, codex uses the user's own `~/.codex`. The `-c` pins still override its top-level `approval_policy` and `sandbox_mode`, so a broader `sandbox_mode` set there, such as `danger-full-access`, is narrowed to `workspace-write` while full access is on. The rest of that directory still applies, including `[sandbox_workspace_write]` settings like `writable_roots` and `network_access` and any command rules, and a user who controls that home directory is also the account the agent runs as. On shared or locked-down deployments, prefer API-key auth (so `CODEX_HOME` is isolated) and keep `NBI_ACP_MODE_POLICY=force-off` unless the agent is explicitly needed.
...
- The pins rely on codex honoring the command-line `-c approval_policy` and `-c sandbox_mode` overrides above its user and project configuration. NBI does not parse or strip codex's own config; managed configuration outranks the pins, and if a future codex version applies a higher-precedence per-project setting, that assumption would need revisiting.
- A command that matches an allow rule in Codex's command rules (`rules/*.rules` in its config directory) runs outside the sandbox. With API-key auth those rules come from NBI's isolated `CODEX_HOME`; with ChatGPT auth, from the user's `~/.codex`.
- Under full access the workspace is the Jupyter server's root directory. When that root contains the NBI user directory, for example a root of `~`, the writable workspace includes `~/.jupyter/nbi`, where the isolated `CODEX_HOME` config and command rules and NBI's `mcp.json` live. An agent steered into editing those files could widen its own sandbox or add an unsandboxed MCP server on a later launch. Where full access is allowed, prefer a root directory that does not contain `~/.jupyter`.
- Full access depends on Codex being able to enforce its platform sandbox, because under `approval_policy = never` a blocked action cannot be approved to run outside it. Where the sandbox cannot start, for example in a container without unprivileged user namespaces, commands fail; on Windows, Codex treats `workspace-write` as read-only unless its Windows sandbox is configured, so edits are still refused there.
- Keep `NBI_ACP_MODE_POLICY=force-off` where these constraints are not acceptable.
...
Opt-in timing instrumentation for diagnosing slow deployments (internal gateways, EFS/NFS home directories, intercepting proxies). [`performance-diagnostics.md`](performance-diagnostics.md) is the full guide, including the span and event reference, the probe thresholds, and worked diagnoses; this section covers the knobs an operator owns.

**Policy triad.** `NBI_PERF_DIAGNOSTICS_POLICY` accepts `user-choice` (default), `force-off`, and `force-on`. `force-on` is designed for fleet-wide baseline collection: it enables recording for every user, enforces redacted attribute detail, and the UI shows the toggle as locked by the administrator. `NBI_PERF_DIAGNOSTICS` is the corresponding value lock, and `NBI_PERF_PROBE_NETWORK=off` disables only the probe's network check, which is the recommended posture when unauthenticated requests to the model gateway would page your SOC. The probe's network check makes a handful of connections, all to the single configured base URL's host: a raw connection to time DNS/TCP/TLS, a second raw connection to verify the certificate against the default trust store, and an HTTP HEAD (with a GET retry if the endpoint replies 405). Only the HTTP requests carry User-Agent `nbi-perf-probe/<version>`; the two raw TLS connections send no HTTP request at all, so a User-Agent-based allowlist rule alone will not catch them, though a single host-based rule still covers every leg.

**Log placement on network filesystems.** The JSONL sink defaults to `<nbi-dir>/perf/`, which on EFS-mounted homes is the same slow filesystem being diagnosed. Set `NBI_PERF_LOG_DIR` to node-local scratch (for example `/tmp/nbi-perf`) in that case. Writes are batched off the request path, tolerate failures, and the sink disables itself after repeated write errors rather than blocking a turn; retention is capped by total size and age. The first line of each log file records the log directory's own filesystem type so a misplaced log is visible in the file itself.

**Reading the numbers.** The panel now names the dominant phase per turn, so the first-pass triage is reading the Verdict column. The underlying comparison is `active` against the SDK-reported `api` duration: close together means the time is in the gateway or the model, divergent means it is local, and then `connect` (subprocess and CLI cold start, typically node startup plus bundle reads on slow storage), `context_prep` (per-turn rule and skill discovery on the server's home filesystem), and `stalls` (mid-stream gaps, tagged by whether they followed tool use or text) say which. For filesystem suspicion, the probe's fsync latency and sustained-throughput rows for `~/.claude` are the EFS signals; sustained throughput of a few MB/s with high fsync latency on an `nfs4` mount is the burst-credit-exhaustion signature, which is confirmed from CloudWatch (`BurstCreditBalance`) rather than from inside the pod. The probe bands each check and prints that reasoning inline, so an operator does not have to carry the thresholds around.

**Fleet baselining.** `force-on` plus `log_to_file` gives every user's turns as JSON Lines under `NBI_PERF_LOG_DIR`. The schema is documented in [`performance-diagnostics.md`](performance-diagnostics.md#collecting-across-sessions-and-machines) and is stable behind `schema_version`. The first line of each daily file is a `{"meta": ...}` record carrying the log directory's own filesystem type, so a log that landed on the slow filesystem being diagnosed is visible in the log itself. Cancelled turns are recorded with `status: "cancelled"` rather than as fast successes, so they do not drag aggregate percentiles down.

**Data governance.** The recorder enforces a fixed attribute allowlist; prompt/response text, file contents, absolute paths, env var values, exception messages, and hostnames are never recorded. Default `attr_detail: redacted` additionally hashes file basenames and model/tool/server names, including the model name carried in the turn header. The turns report carries no hostname at all: the probe target the confirm dialog names is served on the capabilities response instead, so a report pasted into a ticket cannot leak the gateway. Probe output replaces home paths with `~` and the login name with `~user`, filters mount options down to a fixed safe list (CIFS/SMB options otherwise carry the server address and the mount credential, and the withheld count shows as `+N redacted`), and carries a `contains_internal_hostnames` flag when the network check ran, since the configured gateway host appears in its own rows.

## Configuration readiness

A misconfigured NBI fails at the far end of a chat turn, where the user sees a generic apology suggesting they rewrite their prompt. The real cause (an expired key, a base URL pointing at the wrong path, a CLI that is not on `PATH`, a model id the endpoint no longer serves) goes to the server log, which a notebook user never reads.

`GET /notebook-intelligence/readiness` evaluates that as a set of bounded checks and returns a verdict (`ready`, `degraded`, `not_ready`), a one-line headline, and a row per check. Every row that is not `ok` carries a `remedy` naming the next action; a check that cannot name one does not belong in the set. The same document backs the Status card at the top of NBI Settings, General.

It is deliberately **not** gated behind the perf-diagnostics setting the way the two `/perf` routes are: a user who cannot tell "the admin has not set this up" from "I did something wrong" needs the answer whether or not diagnostics are enabled, and an admin verifying a rollout needs it before anyone has opened the settings dialog. It reports configuration state the authenticated user can already read from the capabilities response and the settings panel, so it exposes nothing new. Exception messages are never included, only exception class names, because provider errors routinely carry the base URL and sometimes the key.

**What runs by default (bills nothing):** which path is serving chat; for the native provider path, whether a provider and model are selected, whether the provider is registered (a provider removed by `disabled_providers` looks identical to a typo from the user's side, so the remedy names both), whether the model list is reachable and non-empty, and whether the selected model id is one the endpoint actually serves; for Claude mode, whether the CLI resolves **and answers `--version`** (a CLI that resolves and then fails to start is the usual cause of a turn hanging on "Thinking...", which a path check alone would call healthy) and whether an Anthropic key is present; for ACP mode, the resolved adapter command, whether `npx` is available when the adapter needs it, and whether an agent key is present.

Missing credentials are reported as a **warning, not a blocker**, in both agent modes. The Claude CLI and the ACP agent can each hold a subscription or OAuth login that NBI cannot see, and calling that configuration broken would be wrong.

**The live check (`POST` with `{"live": true}`) bills**, so it has an off switch. `NBI_READINESS_LIVE_CHECK=off` (traitlet `readiness_live_check_allowed`, default on) refuses it while leaving every other readiness check available, and one live test runs at a time per server: a second concurrent request gets a 429 rather than spending tokens and occupying another executor thread.

It sends one short completion to the configured model and is the only way to establish two things nothing cheaper can see:

- **Whether the endpoint actually streams.** A gateway can accept the connection, return 200s, and still buffer the whole response, so replies appear only when the turn ends. Reported as a warning, since it works, just badly.
- **Whether the endpoint accepts a tool schema.** The request carries a trivial tool definition. If it fails and the same request without tools succeeds, a proxy is stripping or rejecting the `tools` field, which means agent mode and every built-in tool will silently never fire. Reported as blocked.

The live check is skipped in Claude and ACP modes, where the agent owns the model call and there is nothing NBI can exercise on the user's behalf. No model output is retained: the probe counts chunks and discards content, so a readiness document stays safe to paste into a support ticket.
