...
To switch Claude mode from the in-process tools to the relay, add `"jupyter_ui_tools_external": true` to the existing `claude_settings` object in NBI's user config, `~/.jupyter/nbi/config.json`. It has no control in the Settings dialog, and saving the Claude tab keeps it. It takes effect only while **Jupyter UI tools** is enabled in the Claude tab (`nbi:built-in-jupyter-ui-tools` in `claude_settings.tools`). Leave it unset and nothing changes.
