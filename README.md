...
| `NBI_ACP_FULL_ACCESS_POLICY`                   | "Full access" for the ACP agent: edit workspace files and run commands without asking, in Codex's workspace-write sandbox (defaults to `force-off`, so the agent asks before anything beyond trusted read-only commands; `user-choice` lets users opt into unattended runs) |
...
| `NBI_CLAUDE_CHAT_MODEL`                | Claude → Chat model (and Inline chat model, unless pinned separately)        |
